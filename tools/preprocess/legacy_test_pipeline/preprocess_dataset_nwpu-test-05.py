...
def main(input_dataset_path, output_dataset_path, min_size=384, max_size=1920):
    ori_img_path = os.path.join(input_dataset_path, 'images_ff')
    ori_anno_path = os.path.join(input_dataset_path, 'mats_ff')
    ...
    for phase in ['test']:
        sub_save_dir = os.path.join(output_dataset_path, phase)
        sub_save_dir1 = os.path.join(r'data\whitefly', phase.split('.')[0])
        if not os.path.exists(sub_save_dir):
            os.makedirs(sub_save_dir)
        with open(os.path.join(input_dataset_path, '{}.txt'.format(phase))) as f:
            lines = f.readlines()
            for i in lines:
                ...
                # im, points, density_map = generate_image(im_path, min_size, max_size)
                ...
                # 将图片保存到 白粉 虱那里
                # 判断图片文件是否存在
                # 有一部分是both 如果该图片存在 则不进行保存
                if not os.path.exists(os.path.join(sub_save_dir1, name)):
                    im.save(os.path.join(sub_save_dir1, name))
...
if __name__ == '__main__':
    input_dataset_path =  r'data\raw_test\fruit_fly'
    output_dataset_path =  r'data\fruit_fly'
