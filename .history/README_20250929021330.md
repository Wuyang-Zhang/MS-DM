...
│
├─ data/                        # 数据相关
│  ├─ data-used-by-train-val-test/          # 某一类任务的训练/验证/测试数据
│  ├─ data-used-by-train-val-test-another/  # 另一类任务的训练/验证/测试数据
│  ├─ images/                   # 训练辅助用的图像
│  ├─ mats/                     # 训练辅助：生成的 .mat 文件
│  └─ mats-1/                   # 训练辅助：另一批 .mat 文件
│
├─ datasets/                    # 数据集处理
│  ├─ annotation/               # 原始标注（可跳过，直接由 .mat 生成）
│  └─ imgs/                     # 原始图片（可跳过，直接由 .mat 生成）
│
├─ losses/                      # 损失函数模块
│
├─ preprocess/                  # 数据预处理脚本
...
将 图片 ---datasets\imgs和 txt 标注文件

--- datasets\imgs和annotation
